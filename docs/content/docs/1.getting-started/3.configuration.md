...
SQLite 连接器在不同环境中存在限制。有些可用于无服务器环境，而另一些则不行。Nuxt Content 支持四种不同的 SQLite 连接器，以覆盖所有环境：

- `better-sqlite3`：适用于所有 Node 环境、GitHub CI、Vercel CI 和生产环境、Cloudflare CI 流水线等。（**不**适用于 WebContainers、StackBlitz 或 Bun 运行时）
- `sqlite3`：适用于 Node 环境、GitHub CI 和 StackBlitz。（**不**适用于 Vercel 或 Cloudflare）
- `native`：从 Node.js v22.5.0 开始，`node:sqlite` 模块在 Node.js 中原生可用。此连接器适用于所有使用 Node.js 22.5.0 或更新版本的 Node 环境。
- `bun`：使用 Bun 内置的 `bun:sqlite` 模块。部署到 Bun 运行时（例如使用 `bun1.x` 运行时的 Vercel）时需要。使用 Bun 构建时会自动选择，但在为 Bun 运行时目标使用 Node.js 构建时必须显式设置。
...
当通过 Nitro 配置运行时以部署到使用 Bun 运行时的 Vercel 时，连接器会自动检测：
...
::prose-note
当设置了 `sqliteConnector: 'bun'` 但构建在 Node.js 上运行时（在像 Vercel 这样的 CI/CD 流水线中很常见），构建时的内容处理会自动回退到兼容 Node.js 的连接器（例如 `better-sqlite3`）。只有已部署的服务器运行时会使用 `bun:sqlite`。无需额外配置。
::

### `experimental.nativeSqlite`（已弃用，改用 `sqliteConnector`）
