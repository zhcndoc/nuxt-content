...
::tip
`queryCollectionNavigation` 工具同时适用于 Vue 和 Nitro。详细了解在服务器端如何使用，请参阅 [服务器端使用](#服务器端使用)。
::
...
目录级导航字段应定义在 `navigation:` 下。这些值会合并到返回的 `ContentNavigationItem` 中，因此自定义标志也可以放在这里。
...
使用该配置后，`queryCollectionNavigation()` 会返回一个该目录的导航项，并且该项对象上可以使用 `icon`、`section` 和 `badge`。
