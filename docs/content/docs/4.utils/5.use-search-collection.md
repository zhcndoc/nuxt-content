...
- `status`：一个响应式 ref，表示索引状态：`'idle'`、`'loading'`、`'ready'` 或 `'error'`。
- `search(query, opts?)`：执行搜索查询。返回一个包含排序结果的 promise。
  - `query`：搜索字符串。自动支持前缀匹配（输入 `compo` 会匹配 "composable"）。
  - `opts`：（可选）搜索选项：
    - `limit`：最大结果数。默认值为 `20`。
    - `fields`：将搜索限制在特定列（`'title'` 或 `'content'`）。
    - `minTermLength`：跳过短于此值的词项。默认值为 `1`。
    - `weights`：控制排序行为。
      - `title`：标题匹配的提升因子。默认值为 `20`。
      - `content`：内容匹配的提升因子。默认值为 `5`。
      - `heading`：控制标题级别提升的指数（`0.5` = 平方根曲线，`1` = 线性，`0` = 禁用）。默认值为 `0.5`。
    - `snippet`：返回高亮文本摘录。
      - `columns`：对哪些列生成摘录（`['title']`、`['content']`，或两者都生成）。默认值为 `['content']`。
