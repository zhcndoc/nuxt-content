...
`navigation` 对象也是 [`queryCollectionNavigation`](/docs/utils/query-collection-navigation) 所消费的自定义元数据的存放位置。在页面 frontmatter 或 `.navigation.yml` 中，诸如 `section`、`group` 或 `badge` 等字段应嵌套在 `navigation` 下，它们将会暴露在生成的 `ContentNavigationItem` 上。
